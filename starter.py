...
def run():
    ...
    while True:
        ...
        # Choose random image
        if (time.clock() - start > TIME_INTERVAL) and times < RANDOM_TIMES:
            start = time.clock()
            times += 1
            ears = random.choice(images)
        ...
        for (i, rect) in enumerate(rects):
            ...
            #left ear
            left_ear_image = cv2.imread(ears[0], -1)
            ...
            placed = place_ear(image, original_left_width, original_left_height,
            shape, left_ear_image, orig_mask_l, orig_mask_inv_l, True)

            #right ear
            if placed :
                right_ear_image = cv2.imread(ears[1], -1)
